...
const base=process.env.W24_BASE_URL||'http://127.0.0.1:5194/w24.html';
...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 fs.mkdirSync(path.join(__dirname,'../.runtime/four-paths'),{recursive:true});fs.writeFileSync(path.join(__dirname,'../.runtime/four-paths','entry-results.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result));
