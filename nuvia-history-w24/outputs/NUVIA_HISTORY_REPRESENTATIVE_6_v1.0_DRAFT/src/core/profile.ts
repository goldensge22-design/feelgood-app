import {PRACTICE_DOMAINS,AGE_BANDS,type Profile,type ConditionDefinition,type AgeBand} from './types';
...
export function resolveProfile(input:unknown):Profile {
 ...
 if(p.practiceDomain!==undefined)need(p.ageBand==='preschool'&&PRACTICE_DOMAINS.includes(p.practiceDomain as any),'PRACTICE_DOMAIN_REQUIRED');
 ...
 return {learnerId:p.learnerId,alias:typeof p.alias==='string'&&p.alias.trim()?p.alias:'어린이 탐험가',ageBand:p.ageBand as AgeBand,...(p.schoolGrade!==undefined?{schoolGrade:p.schoolGrade as Profile['schoolGrade']}:{}),...(p.practiceDomain!==undefined?{practiceDomain:p.practiceDomain as Profile['practiceDomain']}:{}),attentionSupport:p.attentionSupport as 0|1|2|3,source:p.source as Profile['source']};
}
