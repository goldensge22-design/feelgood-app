import {practiceOutcome} from './preschoolPractice';
...
export function bookModel(r:Run){
 ...
 const pages:BookPage[]=[
  ...
  {number:5,role:'alternateHistory',kind:'fiction',text:r.alternate.status==='recorded'?recorded(t(r.action.status==='recorded'&&r.action.value.actionKind==='preschoolActivity'?'gutenberg.preschool.'+practiceOutcome(r.action.value.methodId):c.alternateKeys[r.alternate.value==='A'?0:1])):missing(r.alternate.status),previews:[]},
