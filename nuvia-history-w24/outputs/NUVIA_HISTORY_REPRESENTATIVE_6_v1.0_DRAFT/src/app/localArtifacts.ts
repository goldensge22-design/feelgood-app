...
export async function saveDrawing(run:Run,png:string){if(!png.startsWith('data:image/png;base64,'))throw Error('LOCAL_DRAWING_REQUIRED');const id='drawing:'+randomId();await artifactStore({id,link:artifactLink(run),kind:'drawing',value:png},run);return id;}
