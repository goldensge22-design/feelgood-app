...
export function Speak({text}:{text:string}){
 ...
 return <button className="kid-speak" aria-label={k('listen')} onClick={()=>{speechSynthesis.cancel();const speech=new SpeechSynthesisUtterance(text);speech.voice=voice;speech.lang='ko-KR';speech.rate=.85;speechSynthesis.speak(speech);}}>♪</button>;
}
export function KidExpression({run,repository,prompt,onSave,reason=false,reference,allowSkip=true}:{run:Run;allowSkip?:boolean;repository:BrowserRepository;prompt:string;onSave:(value:Expression)=>Promise<void>;reason?:boolean;reference?:React.ReactNode}){
 ...
 <nav className="kid-modes">{canRecord&&<button disabled={recording||busy} aria-pressed={draft.mode==='voice'} onClick={()=>setDraft({...draft,mode:'voice'})}>{k('voice')}</button>}<button disabled={recording||busy} aria-pressed={draft.mode==='draw'} onClick={()=>setDraft({...draft,mode:'draw'})}>{k('draw')}</button><details open={draft.mode==='text'||undefined}><summary>{k('other')}</summary><button disabled={recording||busy} onClick={()=>setDraft({...draft,mode:'text'})}>{k('write')}</button></details></nav>
 {draft.mode==='draw'?<div data-play-visual><DrawingInput strict height={800} initialValue={draft.drawing} onChange={drawing=>setDraft((d:typeof draft)=>({...d,drawing}))}/></div>:draft.mode==='voice'?<><PictureArt kind="ask" active={recording}/>{canRecord?<button disabled={busy} onClick={()=>void record()}>{recording?k('stop'):k('record')}</button>:<button onClick={()=>setDraft({...draft,mode:'draw'})}>그림</button>}{audioUrl&&<audio controls src={audioUrl}/>}</>:<div className="kid-writing"><textarea autoFocus aria-label={k('write')} data-user-content value={draft.text} onChange={e=>setDraft({...draft,text:e.target.value})}/></div>}
 {error&&<p role="alert">다시 눌러요</p>}<nav><button className="primary" disabled={!valid||busy||recording} onClick={()=>void save()}>{k('save')}</button>{allowSkip&&<button disabled={busy||recording} onClick={()=>void save(true)}>{k('later')}</button>}</nav></section>;
}
...
export function PreschoolPlay({run,profile,definition,existing,busy,repository,start,resume,viewed,send,error}:{run?:Run;profile:Profile;definition:ConditionDefinition;existing:Run|null;busy:boolean;repository:BrowserRepository;start:()=>Promise<void>;resume:()=>void;viewed:()=>Promise<void>;send:(command:Command)=>Promise<void>;error:string}){
 ...
 {(stage==='intro'||stage==='history'||stage==='alternate'&&!(run?.action.status==='recorded'&&run.action.value.actionKind==='preschoolActivity'))&&<div className="kid-hero"><PictureArt kind={art} active={stage==='alternate'} portrait/>{stage==='history'&&<span className="kid-badge">옛날</span>}{stage==='alternate'&&<span className="kid-badge">상상</span>}</div>}
