...
import {ChoiceCard,SecondaryButton} from '../ui/history/HistoryUI';
...
function PracticeStage({run,busy,send,repository,steps,phase}:{run:Run;busy:boolean;send:(c:Command)=>Promise<void>;repository:BrowserRepository;steps:PracticeStep[];phase:string}){
 ...
 function cards(options:KidOption[],handler:(id:string)=>void,sequence=false){return <div className="kid-choices">{shuffled(options,run.resultId+phase).map(o=><ChoiceCard key={o.id} data-practice-choice={o.id} disabled={busy||(sequence&&selected.includes(o.id))} aria-pressed={sequence?selected.includes(o.id):undefined} onClick={()=>{setFailed(false);handler(o.id);}}><PictureArt kind={o.art}/><span>{k(o.key)}</span></ChoiceCard>)}</div>;}
 ...
 {phase==='context'?<><div className="kid-hero"><PictureArt kind={paper?'paper':'access'} active portrait/><span className="kid-badge">{k('imagine')}</span></div><SecondaryButton className="primary" disabled={busy} onClick={()=>void commit([])}>{k('next')}</SecondaryButton></>:phase==='observe'?<>{goal&&<div className="practice-goal-reminder" aria-label={k(goal.key)} data-goal-id={goal.id}><PictureArt kind={goal.art}/></div>}<OutcomeScene method={method}/><SecondaryButton className="primary" disabled={busy} onClick={()=>void commit([method])}>{k('observe')}</SecondaryButton></>:phase==='revise'?<>{goal&&<div className="practice-goal-reminder" aria-label={k(goal.key)} data-goal-id={goal.id}><PictureArt kind={goal.art}/></div>}<OutcomeScene method={method} compact/>{changing?cards(methods,id=>void commit([id])):<nav><SecondaryButton className="primary" disabled={busy} onClick={()=>void commit([method])}>{k('keep')}</SecondaryButton><SecondaryButton disabled={busy} onClick={()=>setChanging(true)}>{k('change')}</SecondaryButton></nav>}</>:<>
 {domain==='simultaneous'&&steps.some(s=>s.phase==='linkPerson')&&<div className="practice-links" aria-label={ui('kid.linkedPictures')} data-play-visual><PictureArt kind="book"/><span aria-hidden="true">↔</span><PictureArt kind={paper?'printer':'fewer'}/>{steps.some(s=>s.phase==='linkPlace')&&<><span aria-hidden="true">↔</span><PictureArt kind={paper?'press':'place'}/></>}{steps.some(s=>s.phase==='linkMethod')&&<><span aria-hidden="true">↔</span><PictureArt kind={method}/></>}</div>}
 ...
 {phase==='sequence'&&<><div className={'practice-order '+(fullOrder?'is-ready':'')} data-play-visual={fullOrder||undefined} aria-label={ui('kid.placedOrder')}>{selected.map(id=>{const item=sequenceCards(cid,method).find(o=>o.id===id)!;return <SecondaryButton key={id} aria-label={ui('kid.removeItem').replace('{item}',k(item.key))} onClick={()=>setSelected(selected.filter(x=>x!==id))}><PictureArt kind={item.art} portrait={false}/>{fullOrder&&<span>{k(item.key)}</span>}</SecondaryButton>;})}</div>{!fullOrder&&cards(sequenceCards(cid,method),id=>setSelected([...selected,id]),true)}{selected.length>0&&<nav><SecondaryButton className="primary" disabled={busy||selected.length!==3} onClick={()=>void commit(selected)}>{k('try')}</SecondaryButton><SecondaryButton disabled={!selected.length||busy} onClick={()=>setSelected([])}>{k('back')}</SecondaryButton></nav>}</>}
 ...
 {!fullOrder&&<SecondaryButton className="kid-help" disabled={busy} onClick={()=>{setFailed(false);setHelp(!help);if(!help)void send({type:'hint',hintType:phase==='sequence'?'sequenceHelp':'visualGuide',level:1});}}>{help?k('backToQuestion'):k('help')}</SecondaryButton>}
 </>}
 </section>;
}
