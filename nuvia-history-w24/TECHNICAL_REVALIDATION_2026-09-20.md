...
최신 P0 수정: `PRESCHOOL_RENDERING_GUARD_2026-09-20.md`를 따른다. 현재 실행본은 미션 5.2.0 / 콘텐츠 3.3.0이며 아래 복구 당시 검증과 구분한다. 최신 검사는 자동 테스트 282개, 연령별 20개 브라우저 경로·108개 화면 확인이다. 현재 LAN 주소는 `http://192.168.45.172:5192`이다.
...
| Node 테스트 | 270/270 통과(비보안 context UUID 대체 경로와 W24 활동 규칙 검사 포함) |
...
입력 복구, 이유 건너뛰기, 이유와 이야기의 분리, C1/C2 분리, 실제 역사 비교와 이야기 완료는 270개 자동 테스트와 보존된 브라우저 검수 자료에서 확인된다. 이번 새 메인 PC 브라우저 smoke는 앱 시작과 저장 비삭제 경계를 다시 확인했으며, 과거 전체 12개 실제 UI 경로를 새로 수행한 것으로 표시하지 않는다.

첨부 화면을 반영한 질문·선택지·표현 UI 수정과 수정 전후 요구 행동은 `W24_ACTIVITY_UI_REVIEW_2026-09-20.md`에 별도로 기록했다. 모바일 390×844 C2와 데스크톱 1440×900 C1 대표 화면을 실제 빌드에서 다시 확인했다.
