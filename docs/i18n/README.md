...
2. 한국어에 맞춘 고정 너비·고정 높이 박스에 다른 언어를 억지로 넣지 않는다.
3. 제목·본문·버튼·탭·카드·라벨은 문장 길이에 따라 줄바꿈, 높이, 간격과 배치가 자연스럽게 조정되어야 한다.
4. 핵심 문구를 `overflow:hidden`, 말줄임표 또는 `line-clamp`로 숨기지 않는다.
5. 화면에 맞추기 위해 번역 의미를 임의로 삭제하거나 축소하지 않는다. 레이아웃을 먼저 고치고, 번역 조정은 의미를 보존한 현지어 표현으로 별도 검수한다.
6. 텍스트 컨테이너는 콘텐츠에 따라 높이가 늘어나는 구조를 우선한다.
7. Grid·Flex의 텍스트 자식에는 `min-width:0`을 적용하고 필요한 줄바꿈을 허용한다.
8. 긴 단어와 복합어에는 언어 특성에 맞는 `overflow-wrap`, `word-break`, `hyphens`를 적용한다.
9. 버튼은 최소 높이와 두 줄 표시를 허용해 전체 문구를 읽을 수 있게 한다.
10. 제목은 `clamp()` 등으로 반응형 조정하되 지나치게 작게 축소하지 않는다.
11. 해결 순서는 줄바꿈 → 컨테이너 확장 → 열 비율 변경 → 배치 전환 → 제한적 글자 크기 조정이다.
12. 공통 구조를 먼저 고치고 꼭 필요한 예외만 `html[lang]`, `:lang()` 또는 기존 locale class로 조정한다.
13. RTL은 방향·정렬·아이콘·여백·열 배치를 별도 검증한다.
14. 태국어·크메르어 등 고유한 줄바꿈과 글꼴 특성이 있는 언어는 실제 브라우저에서 확인한다.
15. 다국어 QA는 key·빈 값·placeholder뿐 아니라 실제 DOM의 잘림·겹침·가로 overflow·부모 경계 이탈을 검사한다. 의도된 문서 세로 스크롤은 허용한다.
16. 신규 문장이나 번역 변경 뒤에는 manifest의 지원 언어 전체에서 해당 컴포넌트를 다시 검사한다.
17. 한 언어의 수정이 다른 언어의 화면을 깨뜨리면 완료로 판정하지 않는다.
18. 이 규칙은 전자책·제안서·결과지·대시보드·NUVIA 프로그램 등 모든 최종 사용자 화면에 적용한다.
