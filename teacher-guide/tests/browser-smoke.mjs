...
const routes = ['opening','guide','assessment','assessment-expertise','pass','pathways','results','profiles','teacher','dashboard','school-dashboard','nuvia','parents','support','closing'];
const viewports = [
...
try {
  ...
  await waitFor("document.querySelector('#profileAnalysis .profile-code')?.textContent==='P-M / A-M / S-M / Q-M' && document.querySelector('#teacherProfileAnalysis .profile-code')?.textContent==='P-M / A-M / S-M / Q-M' && document.querySelector('#parentProfileAnalysis .profile-code')?.textContent==='P-M / A-M / S-M / Q-M' && document.querySelectorAll('[data-language-select] option').length===15", 'profile engines and language data');
  ...
  if (report.static.chapters !== 15 || report.static.toc !== 15 || report.static.duplicates.length || report.static.emptyHref || report.static.brokenInternal.length || report.static.stylesheets.join() !== 'ebook.css' || report.static.internalStatusDom) failures.push('static-integrity');
  ...
  await openRoute('school-dashboard');
  report.interactions.schoolDashboard = await evaluate("(() => { const link=document.querySelector('[data-school-dashboard-link]'); const url=new URL(link.href); return {path:url.pathname,lang:url.searchParams.get('lang'),target:link.target,rel:link.rel}; })()");
  await openRoute('teacher');
  ...
  const languagePass = report.interactions.language.count===15 && report.interactions.language.unique===15 && report.interactions.language.ready.join()==='ko' && report.interactions.language.drafts.length===14 && !report.interactions.languageTransition.koreanFlash && report.interactions.languageTransition.finalLanguage==='ru' && languageResults.every(item => !item.documentOverflow && item.clipped===0 && item.hash==='#teacher' && item.profileCode===languageSetup.profile && item.teacherCode===languageSetup.teacher && item.parentCode===languageSetup.parent && item.profileValues==='H,M,L,H' && item.teacherValues==='H,L,M,H' && item.parentValues==='L,H,M,L' && item.dashboardPressed && item.reportLangs.every(code=>code===item.code) && item.schoolDashboardLang===item.code && item.userSurface.forbiddenHits.length===0 && item.userSurface.exactStatus.length===0 && item.userSurface.internalElements.length===0 && item.userSurface.cleanTopFlow && (item.code==='ko' ? item.dir==='ltr' && item.teacherHasKorean && item.parentHasKorean && item.guideHasKorean && item.reset===languageSetup.koreanReset && item.heading===languageSetup.koreanHeading : !item.teacherHasKorean && !item.parentHasKorean && !item.guideHasKorean && item.dir===(item.code==='ar'?'rtl':'ltr') && item.reset!==languageSetup.koreanReset && item.heading!==languageSetup.koreanHeading));
  const schoolDashboardPass = report.interactions.schoolDashboard.path.endsWith('/school-dashboard-demo/track.html') && report.interactions.schoolDashboard.lang==='ko' && report.interactions.schoolDashboard.target==='_blank' && report.interactions.schoolDashboard.rel.includes('noopener') && report.interactions.schoolDashboard.rel.includes('noreferrer');
  if (report.interactions.openingPathways !== '#pathways' || report.interactions.openingTeacher !== '#teacher' || !profilesPass || !teacherPass || !parentPass || !report.interactions.dashboard.changed || !report.interactions.dashboard.pressed || !schoolDashboardPass || !languagePass || !resultGuidesPass) failures.push('interactions');
  ...
  const output = process.env.TEACHER_GUIDE_QA_SUMMARY === '1' ? {
    ...
    interactions:{
      ...
      schoolDashboard:report.interactions.schoolDashboard,
