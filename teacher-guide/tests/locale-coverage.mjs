...
for (const language of drafts) {
  try {
    ...
    const valid = pack.locale === language.code && pack.sourceLocale === 'ko' && pack.status === 'ai-draft' && pack.direction === direction && pack.sourceHash === expectedHash && sameKeys && !empty.length;
    results[language.code] = {messages:keys.length,empty:empty.length,sameKeys,direction:pack.direction,valid};
...
console.log(JSON.stringify({languages:manifest.length,ready,drafts:drafts.length,expectedMessages:expectedKeys?.length || 0,results,status:failures.length ? 'FAIL: ' + failures.join(', ') : 'PASS'}, null, 2));
